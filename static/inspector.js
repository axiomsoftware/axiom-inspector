...
var axiom = {
	...
	init: function() {
		...
		axiom.tree.on({	click: function(node, evt){
						  ...
						  contextmenu: function(node, evt){
							  ...
							  for(var i=0; i<prototypes.length;i++){
								  if(i%20 == 0 && i > 0){
									  var sub = {text: "More", menu: []};
									  current_menu.push(sub);
									  current_menu = sub.menu;
								  }
								  current_menu.push({text: prototypes[i],
							  ...
							  var contextmenu = new Ext.menu.Menu({ items: [{text: "Add Child", menu: menu},
																			{text: "Refresh", handler: function(){
																				if(node.reload){ 	// nodes without children aren't AsyncTreeNodes
																					node.reload();
																				}
																			}},
																			{text: "Delete",
		...
		var clearConf = {text:'clear',
						 handler:function(){ shellHistoryStore.removeAll(); }
						};
		...
		var shellgrid = new Ext.grid.GridPanel({ id: 'shell',
												 ...
												 tbar : ["Javascript Shell", "->",
														 new Ext.Button({text:'logout',
																		 handler: function(){
																			 Ext.Ajax.request({ url: handler_url,
